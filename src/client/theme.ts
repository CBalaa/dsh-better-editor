/**
 * Live theme access for surfaces that need concrete color values — monaco's
 * theme payloads — while the app's scheme flips at runtime. Mirrors the
 * subset of dsh-better-sidebar's theme helpers this plugin consumes: the
 * resolved scheme, computed token values, the translucent-token guard
 * (issue-#90 glass skins), and the scheme-flip subscription.
 */

/** Whether the app shell resolved to the dark scheme (mirror of the sidebar's). */
...
/** The alpha channel of a computed CSS color, or null when unparseable. */
...
/** A token value that actually PAINTS something; '' for visually inert values. */
